      ...
      <section className="mb-10">
        ...
        {club.matchFn(0, club.currency) === false && club.slug === "dc" ? (
          <div className="rounded-lg bg-amber-50 border border-amber-200 p-4 text-amber-800 text-sm">
            ...
            attributed to NYC (the only existing $26 Stripe product). Create a separate DC product +
            payment link in Stripe, then update <code className="bg-amber-100 px-1 rounded">lib/clubs.ts</code> to match it.
          </div>
        ) : stripeError ? (
          <div className="rounded-lg bg-red-50 border border-red-200 p-4 text-red-700 text-sm">
            {stripeError} — ensure{" "}
            <code className="bg-red-100 px-1 rounded">STRIPE_SECRET_KEY</code> is set.
          </div>
        ) : (
